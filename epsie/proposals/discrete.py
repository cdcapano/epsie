...
class NormalDiscrete(Normal):
    ...
    where :math:`\Delta x \sim \mathcal{N}(0, \sigma)`. This results in a
    stepped probability density that is zero :math:`\in [0, 1)`. In other
    words, the proposal will never produce the same integer on successive
    jumps, and it will most often draw integers closest to the current point.
