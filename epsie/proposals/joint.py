...
class TransdimensionalProposalsList(object):
    """A utility object holding proposals used for RJMCMC. In particular,
    keeps track of which proposals are currently active and inactive and
    offers quick numpy functionality in toggling some proposals on and off
    and returning active and inactive proposals.
    ...
    def proposals(self, proposals):
        if any([prop.active is None for prop in proposals]):
            raise ValueError("all passed in proposals must be either "
                             "active or inactive")
        self._active = numpy.array([prop.active for prop in proposals])
        # store the proposals
    ...
    def active_proposals(self, mask=None):
        print(mask)
        if mask is None:
            return self._proposals[self._active]
        else:
            return self._proposals[mask]

    def inactive_proposals(self, mask):
        print(mask)
        if mask is None:
            return self._proposals[numpy.logical_not(self._active)]
        else:
            return self._proposals[numpy.logical_not(mask)]

    def flip_proposals(self, i):
        if isinstance(i, (list, tuple)):
            i = numpy.array(i)
        self._active[i] = numpy.logical_not(self._active[i])
